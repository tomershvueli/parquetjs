...
exports.shredRecord = function(schema, record, buffer) {
  ...
  for (let field of schema.fieldList) {
    ...
    let rlevels_length = record.rlevels.length;
    for (let i = 0; i < rlevels_length; i++) {
    ...
    for (let value of record.distinct_values.values()) {
      column.distinct_values.add(value)
    }
...
function shredRecordInternal(fields, record, data, rlvl, dlvl) {
  for (let fieldName in fields) {
    ...
    // push null
    let data_field = data[field.path];
    if (values.length == 0) {
      if (field.isNested) {
        shredRecordInternal(
            field.fields,
        ...
        data_field.rlevels.push(rlvl);
        data_field.dlevels.push(dlvl);
        data_field.count += 1;
      }
      continue;
    }
    ...
    for (let i = 0; i < value_length; ++i) {
      ...
      if (field.isNested) {
        ...
        data_field.distinct_values.add(values[i]);
        if(fieldType == "UTF8") {
          data_field.values.push(values[i]);
        } else {
          data_field.values.push(parquet_types.toPrimitive(fieldType, values[i]));
        }
        data_field.rlevels.push(rlvl_i);
        data_field.dlevels.push(field.dLevelMax);
        data_field.count += 1;
      }
    }
  }
